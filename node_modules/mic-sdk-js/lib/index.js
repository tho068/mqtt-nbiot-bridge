...
class MIC {
  ...
  _parseError(e) {
    if (e && e.errorMessage) return JSON.parse(e.errorMessage).message;else if (typeof e === 'string') return JSON.parse(e);else return e;
  }
  ...
  _isAuthError(e) {
    ...
    return typeof e === 'string' && e.match(authErrors) || typeof e.message === 'string' && e.message.match(authErrors);
  }
  ...
  async _loadManifest() {
    ...
    if (this._host === null) throw `Filed to load manifest: MIC is not initialized.`;

    try {
      const result = await (0, _axios2.default)(manifestEndpoint);
  ...
  async invoke(functionName, payload) {
    ...
    if (!this._assertInited()) throw `Filed to load manifest: MIC is not initialized.`;
  ...
  async _lambda(functionName, functionPayload) {
    try {
      ...
      // Empty response
      if (!result || !result.Payload) throw 'Error while invoking Lambda: No response data.';

      // No error, got a response
      const payload = JSON.parse(result.Payload);

      // Got an error message in response
      if (result.FunctionError || payload.errorMessage) throw this._parseError(payload);
  ...
  async _getCredentials(token = null) {
    try {
      ...
      if (token === null) this._AWS.config.credentials.clearCachedId();
  ...
  async login(username, password) {
    try {
      if (!this._assertInited()) throw `Filed to load login: MIC is not initialized.`;
