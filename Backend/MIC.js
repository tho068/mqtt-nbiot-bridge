...
class MIC {
  ...
  init() {
    return this.loadManifest()
      ...
      .catch(error => logger.error('-- MIC: error,', error))
  }
  ...
  /* Get certificate files for a given thingId */
  certificate(thingId, token) {
    return new Promise((resolve, reject) => {
      this.init().then(() => {
        this.getCredentials(token)

        this.invoke("ThingLambda", {
